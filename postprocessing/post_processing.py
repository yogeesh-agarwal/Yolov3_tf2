...
from Yolov3_tf2.postprocessing import utils as pp_utils
...
def gen_box_objects(final_boxes , final_probs , final_class , gt = False):
    ...
    for i in range(len(final_boxes)):
        box = comman_utils.BoundingBox(final_boxes[i][0],
                          ...
                          center = True,
                          gt = gt)
        label = class_names[final_class[i]]
        conf = final_probs[i]
        box.add_class(label)
...
def post_process(predictions,
    ...
    for i in range(num_images):
        ...
        if show_image:
            ...
            draw_predictions(images , final_boxes , final_probs , final_class , class_names)
